...
def login_user():
    if user.get() == '' or user.get() == 'Username' or code.get() == '':
        ...
        try:
            ...
            con = pymysql.connect(
                host='sql12.freesqldatabase.com',  # Replace with your cloud database host
                user='sql12776990',                # Your database username
                password='A22rYY62nK',             # Your database password
                database='sql12776990',            # Your database name
                port=3306                          # Optional: Default MySQL port
            )
            mycursor = con.cursor()
        except Exception as e:
            messagebox.showerror("Error", f"Connection Failed With Database: {e}")
            return
...
def forgot_password():
    ...
    fwindow = Toplevel()
    # (UI setup code for the forgot password window remains unchanged)
